<template>
  <div class="dropdown" :class="{ show: isListShow }">
    <button type="button" class="button dropdown__toggle"
      :class="{ dropdown__toggle_icon: isIcon }"
      @click="isListShow = !isListShow">
      <app-icon :icon="icon" v-if="icon" />
      {{ title }}{{ text }}
    </button>

    <div class="dropdown__menu" :class="{ show: isListShow }">
      <button class="dropdown__item" :class="{ dropdown__item_icon: isIcon }" type="button"
      v-for="(option, index) in options" :key="option.value" @click="change(option.value)">
...
export default {
  ...
  methods: {
    getOption() {
      return this.options.filter(elem => elem.value === this.value)[0];
    },
    change (value) {
  ...
  computed: {
    text() {
      return this.value ? ` - ${this.getOption(this.value).text}` : ``;
    },
    icon() {
      const option = this.value ? this.getOption(this.value) : {};
